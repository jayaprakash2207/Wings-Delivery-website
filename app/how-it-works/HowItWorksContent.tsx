...
const steps = [
  {
    ...
    description: 'Open WhatsApp and send a message to +91 76038 14870. You can type in Tamil, Tanglish, Telugu, or English. Even a voice note works!',
  },
  {
    number: '02',
  ...
  return (
    ...
    >
      <button
        ...
        aria-expanded={open}
      >
        <span className="font-barlow font-bold text-sm md:text-base uppercase text-[#EFF2F7] pr-4">{q}</span>
        <ChevronDown
          size={18}
            ...
            {steps.map(({ number, title, description }, i) => (
              ...
              >
                ...
                <div className="flex-1 pt-3 pb-4">
                  <h3 className="font-barlow font-black text-xl md:text-2xl uppercase text-[#EFF2F7] mb-2">{title}</h3>
                  <p className="font-inter text-sm md:text-base text-[#7E8A9E] leading-relaxed max-w-lg">{description}</p>
                </div>
              </motion.div>
            ))}
          </div>
                    ...
                    <div key={i} className={`flex ${msg.from === 'user' ? 'justify-end' : 'justify-start'}`}>
                      <div
                        className={`max-w-[80%] px-3 py-2 rounded-2xl text-xs font-inter leading-relaxed ${
                          msg.from === 'user' ? 'bg-[#25D366]/20 text-[#EFF2F7] rounded-br-sm' : 'text-[#EFF2F7] rounded-bl-sm'
                        }`}
