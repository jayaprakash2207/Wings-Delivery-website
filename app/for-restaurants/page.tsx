...
const benefits = [
  {
    icon: TrendingUp,
    title: 'More Orders',
    description: 'Reach 30+ villages you couldn\'t serve before. Our batch delivery brings your food to new customers every day.',
  },
  {
    icon: Package,
    title: 'No Delivery Headache',
    description: 'You cook, we deliver. No need for your own delivery staff or vehicles. Focus on what you do best.',
  },
  {
    icon: BarChart2,
    title: 'Simple Dashboard',
    description: 'Receive orders clearly via WhatsApp. Know exactly what to prepare and when. No complicated software.',
  },
  {
    icon: DollarSign,
    title: 'Weekly Settlements',
    description: 'Transparent weekly payouts directly to your bank account. Clear commission structure, no hidden fees.',
  },
]
...
export default function ForRestaurantsPage() {
  const [form, setForm] = useState({
    restaurant: '', owner: '', phone: '', location: '', cuisine: '',
  })
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {benefits.map(({ icon: Icon, title, description }, i) => (
              <motion.div
                key={title}
                className="card rounded-[14px] p-6 hover:-translate-y-1 transition-transform duration-300"
      ...
      <section style={{ background: '#141825' }} className="py-20 md:py-28">
        <div className="max-w-2xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-10">
            ...
            <p className="font-inter text-[#7E8A9E] text-sm mt-3">
              Fill this form and we&apos;ll reach out within 24 hours.
            </p>
          </div>
          <form onSubmit={handleSubmit} noValidate className="card rounded-2xl p-8 flex flex-col gap-5">
            ...
            <div>
              ...
              >
                ...
                {cuisines.map((c) => (
                  <option key={c} value={c} style={{ background: '#141825' }}>{c}</option>
                ))}
              </select>
            </div>
            <button
          ...
          <div className="text-center mt-6">
            ...
            <a
              href="https://wa.me/919123538768?text=Hi%20Wings%20Delivery!%20I%20want%20to%20become%20a%20restaurant%20partner."
              target="_blank"
