...
const contactCards = [
  { icon: Phone,     label: 'WhatsApp',  value: '+91 76038 14870',                                                  href: 'https://wa.me/917603814870',               color: '#25D366' },
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
            <motion.div className="card rounded-2xl p-5 sm:p-8" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }}>
              ...
              <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-5">
                <div>
                  <label className="font-barlow font-bold text-xs uppercase tracking-[0.14em] text-[#7E8A9E] block mb-2">Full Name *</label>
                ...
                <div>
                  <label className="font-barlow font-bold text-xs uppercase tracking-[0.14em] text-[#7E8A9E] block mb-2">Phone Number *</label>
                ...
                <div>
                  <label className="font-barlow font-bold text-xs uppercase tracking-[0.14em] text-[#7E8A9E] block mb-2">Message *</label>
                ...
                <button type="submit" disabled={loading}
                  ...
                  {loading ? <span className="inline-block w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <Send size={16} />}
                  {loading ? 'Opening WhatsApp...' : 'Send via WhatsApp'}
                </button>
                <p className="font-inter text-xs text-[#7E8A9E] text-center">This will open WhatsApp with your message pre-filled.</p>
              </form>
            </motion.div>

            <motion.div className="flex flex-col gap-4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.1 }}>
              {contactCards.map(({ icon: Icon, label, value, href, color }) => (
                <div key={label} className="card rounded-xl p-5 flex items-start gap-4 hover:-translate-y-0.5 transition-transform duration-250">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
                    style={{ background: `${color}12`, border: `1px solid ${color}22` }}>
                    <Icon size={18} style={{ color }} />
                  </div>
                  <div>
                    <p className="font-barlow font-semibold text-xs uppercase tracking-[0.14em] text-[#7E8A9E] mb-0.5">{label}</p>
                    {href ? (
              ...
              <div className="rounded-2xl overflow-hidden h-48 mt-2" style={{ border: '1px solid rgba(255,255,255,0.07)' }}>
                <iframe
                  ...
                  title="Wings Delivery Location — Uthukottai, Tiruvallur, Tamil Nadu" />
              </div>
            </motion.div>
          </div>
        </div>
